...
function AdminProducts() {
    ...
    // Dùng useCallback + memoize userId & role
    ...
    }, [userId, role]); // ← CHỈ PHỤ THUỘC userId & role
    ...
    useEffect(() => {
        ...
        fetchProducts();
    }, [fetchProducts, navigate, userId, role]); // ← ĐÚNG DEPENDENCY
        ...
        try {
            ...
            if (imageFile) {
                ...
                const uploadRes = await axios.post('http://localhost:5000/api/upload', uploadData, {
                    headers: { 'user-id': userId, 'role': role, 'Content-Type': 'multipart/form-data' },
                });
    ...
    const handleUpdateProduct = async (e) => {
        e.preventDefault();
        if (!window.confirm('Cập nhật sản phẩm?')) return;
        if (loading) return;
        setLoading(true);
        try {
            let imageUrl = formData.imageUrl;
                                    ...
                                    <tr key={product._id}>
                                        <td>
                                            <img
                                                ...
                                                alt={product.name}
                                                className="product-image product-image_admin"
                                            />
                                        </td>
                                        <td>{product.name}</td>
